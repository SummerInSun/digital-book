...
static int AsciiFontInit(char *pucFontFile, unsigned int dwFontSize);
static int AsciiGetFontBitMap(unsigned int dwCode, struct FontBitMap *ptFontBitMap);

struct FontOpr g_tAsciiFontOpr = {
	.name = "ascii",
...
{
	if(dwFontSize != 16){
		DebugPrint(DEBUG_NOTICE"ascii font can only support '16' size\n");
		return -1;
	}
	return 0;
}
...
static int AsciiGetFontBitMap(unsigned int dwCode, struct FontBitMap *ptFontBitMap)
{
	...
	if(dwCode > 127){
		DebugPrint(DEBUG_NOTICE"Ascii font has no this code %d\n", dwCode);
		return -1;
	}
