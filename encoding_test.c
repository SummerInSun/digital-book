...
{  
	int i;
	int iError = 0;
	unsigned char ucUtf8CodeStr[] = {0xef, 0xbb, 0xbf, 0x7f, 0xf0, 0xb0, 0};
	unsigned int dwUtf8CodeTest;

	FBInit();
	FontInit();
	EncodingInit();

	g_tTestEO = GetEncodingOpr("uft16be-encoding");

	g_ptFTFontOpr = g_tTestEO->ptFontOprSupportedHead;

	while(g_ptFTFontOpr){
		printf("%s\n\n", g_ptFTFontOpr->name);
		g_ptFTFontOpr = g_ptFTFontOpr->ptFontOprNext;
	}

	g_ptFTFontOpr = g_tTestEO->ptFontOprSupportedHead;
	DelFontOprForEncoding(g_tTestEO, g_ptFTFontOpr);
	g_ptFTFontOpr = g_tTestEO->ptFontOprSupportedHead;
	while(g_ptFTFontOpr){
		printf("%s\n\n", g_ptFTFontOpr->name);
		g_ptFTFontOpr = g_ptFTFontOpr->ptFontOprNext;
	...
	iError = g_tTestEO->GetCodeFromText(&ucUtf8CodeStr[3], &ucUtf8CodeStr[6], &dwUtf8CodeTest);
	printf("\nUtf8 code num %d, code 0x%x\n", iError, dwUtf8CodeTest);
	...
	g_ptFTFontOpr->GetFontBitMap(dwUtf8CodeTest, &g_ptFTFontBitMap);
/*
	for(i = 0; i < 240; i ++){
